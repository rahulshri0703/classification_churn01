...
pipeline = Pipeline(
    [
        ...
        (
            'nullZero',
            pp.sklearnWrapper(features=config.model_config.nullFillZero,
        ...
        (
            'nullMode',
            pp.sklearnWrapper(features=config.model_config.nullFillMode,
                              transformer=SimpleImputer(
        ...
        (
            ...
            RareLabelEncoder(tol=0.01, n_categories=3,
                             variables=config.model_config.categorical
                             )
        ),
        (
            'one',
        ...
        (
            ...
            Winsorizer(capping_method='gaussian', tail='both',
                       fold=3, variables=config.model_config.numerical
                       )
        ),
        (
            'scale',
            pp.sklearnWrapper(features=config.model_config.numerical,
